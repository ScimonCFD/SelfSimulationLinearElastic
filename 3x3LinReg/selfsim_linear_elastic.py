...
#Run the simulation with the theoretical material model
...
# Bring the linear regression model and the scalers to the parent folder
terminal("cp " + ROUTE_TO_NEURAL_NETWORK_CODE + "ML_model.joblib " + 
         ROUTE_NN_MODEL) 
terminal("cp " + ROUTE_TO_NEURAL_NETWORK_CODE + "*scaler.joblib " + 
         ROUTE_NN_MODEL)

# Load the initial linear regression model
ML_model = load(ROUTE_NN_MODEL + 'ML_model.joblib')
...
for pass_number in range(TOTAL_NUMBER_PASSES):
    for load_inc in range(TOTAL_LOAD_INCREMENTS):
        for i in range(TOTAL_ITERATIONS):
            ...
            # Serialise the flag
            serialise_word_or_list(flag, ROUTE_NN_MODEL, "flag")
            # Run simulation A
            terminal("cd " + ROUTE_NN_MODEL + " && ./Allrun")
            
            # Copy DExpected to load_inc + 1 results folder
            terminal("cd " + ROUTE_THEORETICAL_MODEL + str(load_inc + 1)  
                      + "/ && cp D ../../../" + ROUTE_NN_MODEL 
                      + str(load_inc + 1) + "/DExpected")
            
            # Copy sigmaExpected to load_inc + 1 results folder
            terminal("cd " + ROUTE_THEORETICAL_MODEL + str(load_inc + 1) 
            ...
            # previous lines, create Python files for those fields (serialise 
            #those fields)
            ...
            if(load_inc == 0):
                ...
                change_bc_from_traction_to_d(ROUTE_NN_MODEL + str(load_inc), 
                                             True)
            ...
            terminal("cd " + ROUTE_NN_MODEL + " && cp -r " 
                     + str(load_inc + 1) + " " + str(load_inc + 1) + "B" )
            ...
            # Reduce the OpenFOAM field. 6 columns --> 3 columns
            new_epsilon = np.zeros([epsilon.shape[0], 3])
            new_sigma = np.zeros([sigma.shape[0], 3])
            
            new_epsilon[:, 0] = epsilon[:, 0]
            new_epsilon[:, 1] = epsilon[:, 1]
            new_epsilon[:, 2] = epsilon[:, 3]
            
            new_sigma[:, 0] = sigma[:, 0]
            new_sigma[:, 1] = sigma[:, 1]
            new_sigma[:, 2] = sigma[:, 3]
        
            # Load master_x_training_set and master_x_training_set
            if (pass_number == 0 and load_inc == 0 and i == 0):
                ...
                if (i == 0):
                    master_x_training_set = deserialise(
                        ROUTE_NN_MODEL, "master_x_training_set")
                    
                    master_y_training_set = deserialise(
                        ROUTE_NN_MODEL, "master_y_training_set")   
                ...
                if (sigma.shape[0] * SETS_IN_MOVING_WINDOW > \
                    ...
                    x_training_set = \
                        np.concatenate((master_x_training_set[\
                            ...
                            axis=0)
                    y_training_set = \
                        np.concatenate((master_y_training_set[\
                            (-1* SETS_IN_MOVING_WINDOW * \
                              new_sigma.shape[0]):, :], new_sigma), 
                            axis=0)      
            ...
            # Update the ML model
            ML_model = create_linear_reg(epsilon_scaled, sigma_scaled)
            ...
            diff = (mae(D_simul_A[:, :2], D_expected[:, :2]).numpy())/\
                (np.sum(D_expected[:, :2] * D_expected[:, :2]))**0.5
            ...
            if (i!=(TOTAL_ITERATIONS-1) and (diff > TOL_LOCAL_ITER)):
                ...
                master_x_training_set = np.concatenate(
                    (master_x_training_set, new_epsilon), axis=0)
                master_y_training_set = np.concatenate(
                    (master_y_training_set, new_sigma), axis=0)
...
# print("Total calculation time: ", (end-start), " (s) \n or \n ", (end-start)/3600, "(h)")