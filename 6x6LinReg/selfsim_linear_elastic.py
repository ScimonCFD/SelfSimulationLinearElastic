...
if (SUBSAMPLE_ORIGINAL_STRAINS):
    original_x_training_set = deserialise(ROUTE_TO_NEURAL_NETWORK_CODE, "x_train")
    ...
    for load_inc in range(TOTAL_LOAD_INCREMENTS):
        for i in range(TOTAL_ITERATIONS):
            ...
            terminal("cd " + ROUTE_THEORETICAL_MODEL + str(load_inc + 1) + 
                     "/ && cp D ../../../" + ROUTE_NN_MODEL+ str(load_inc + 1) +
                     "/DExpected")
            ...
            terminal("cd " + ROUTE_NN_MODEL + " && rm -r " + str(load_inc + 1) + 
                     " log* ")
            ...
            # 'BoundaryChange' and will set the values at the boundaries as 
            ...
            if (i == 0):
                sigma_expected = deserialise(ROUTE_NN_MODEL  + str(load_inc + 1) 
                                             + "a" +  "/", "sigmaExpected")
            
                #Load epsilon from simul B
                epsilon_expected = deserialise(ROUTE_NN_MODEL + str(load_inc + 1) 
                                               + "B" +  "/", "epsilonExpected")    
            ...
            if (i!=(TOTAL_ITERATIONS-1) and (diff > TOL_LOCAL_ITER)):
                ...
                terminal("cd " +  ROUTE_NN_MODEL + str(load_inc) + 
                          "/ && cp D_original ./D")        ######1 
                print(" \n \n \n MAE(expected vs calculated D) at this iteration is ", 
                      diff, "\n \n \n")        
                ...
                terminal("cp " + ROUTE_NN_MODEL + "*.joblib " +" " + 
                         route_store_results + "/")
                terminal("cp -r " + ROUTE_NN_MODEL + "*a " + ROUTE_NN_MODEL +"*B " 
                         + " " + route_store_results)
